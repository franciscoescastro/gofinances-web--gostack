  ...
  useEffect(() => {
    async function loadTransactions(): Promise<void> {
      api.get('/transactions').then(response => {
        ...
        transactionsResponse.forEach((transaction: Transaction) => {
          const createAt = new Date(transaction.createdAt)
          transaction.formattedValue =
            (transaction.type === 'outcome' ? '- ' : '') +
            formatValue(transaction.value)
          transaction.createdAt = createAt
          transaction.formattedDate = `${createAt.getDay()}/${createAt.getMonth()}/${createAt.getFullYear()}`
